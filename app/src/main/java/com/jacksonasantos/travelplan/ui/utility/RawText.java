...
public class RawText {
    ...
    */
    public int addToString(String newString, int start, int maxLength) {
        ...
        if(newString == null || newString.equals("")) { return 0; }
        else if(start < 0) {throw new IllegalArgumentException("Start position must be non-negative"); }
        else if(start > text.length()) { throw new IllegalArgumentException("Start position must be less than the actual text length"); }
    ...
    public char charAt(int position) {return text.charAt(position); }
}